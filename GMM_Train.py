...
### Gaussian Mixture Model for unchanging Cov and prior
...
for k in range(2,6):
    ...
    errors = []
    for i in range(20):
        gmm = GMMu(final_data_scaled1, k, 0.01)
        preds = gmm.fit()
        errors.append(sc(final_data_scaled1, preds))
    total_errors.append(errors)
    
### plotting score
errors_nu = pd.DataFrame(total_errors)
errors_nu = errors_nu.transpose()
errors_nu.columns = [2,3,4,5]
...
### plotting the score
errors_nu_db = pd.DataFrame(total_errors)
errors_nu_db = errors_nu_db.transpose()
errors_nu_db.columns = [2,3,4,5]

plt.figure(figsize=(10,10))
errors_nu_db.boxplot()
plt.ylabel('Score')
plt.xlabel('Cluster Count')
plt.show()
