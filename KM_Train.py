...
for k in range(2,6):
    ...
    errors = []
    for i in range(20):
        gmm = GMM(final_data_scaled1, k, 0.01)
        preds = gmm.fit()
        errors.append(sc(final_data_scaled1, preds))
    total_errors.append(errors)
    
### plotting score
errors_gm = pd.DataFrame(total_errors)
errors_gm = errors_gm.transpose()
errors_gm.columns = [2,3,4,5]
...
### plotting score
errors_gm_db = pd.DataFrame(total_errors)
errors_gm_db = errors_gm_db.transpose()
errors_gm_db.columns = [2,3,4,5]

plt.figure(figsize=(10,10))
errors_gm_db.boxplot()
plt.ylabel('Score')
plt.xlabel('Cluster Count')
plt.show()
...
for k in range(2,6):
    ...
    for i in range(20):
        km = kmeans(k)
        preds = km.fit(final_data_scaled1, 0.01)
        errors.append(sc(final_data_scaled1, preds))
    total_errors_k.append(errors)
    
### plotting score
errors_km = pd.DataFrame(total_errors_k)
errors_km = errors_km.transpose()
errors_km.columns = [2,3,4,5]
...
### plotting score
errors_km_db = pd.DataFrame(total_errors_k)
errors_km_db = errors_km_db.transpose()
errors_km_db.columns = [2,3,4,5]

plt.figure(figsize=(10,10))
errors_km_db.boxplot()
plt.ylabel('Score')
plt.xlabel('Cluster Count')
plt.show()
